...
Some data (like LD-scores) are required for PleioVar and its dependencies. 
...
Let's format the LHC-MR results in a PleioVar-friendly format, and gather the last necessary inputs.
```{r pv1, eval = FALSE}
...
If you have a Zscore column in your GWAS data, PleioVar will process them.  
If you do not, the Zscores will be computed with $tstat/\sqrt{(sample \: size)}$. If your formatted your data according to LHC-MR guidelines and the previous steps worked, everything should run automatically.
